...
def Summarization():
    ...
    for epoch in range(cfg.num_epochs):
        ...
        for batch in train_loader:
            if isinstance(batch, (list, tuple)) and len(batch) == 2:
                src, tgt = batch
                src = torch.tensor(src).to(cfg.device) if isinstance(src, list) else src.to(cfg.device)
                tgt = torch.tensor(tgt).to(cfg.device) if isinstance(tgt, list) else tgt.to(cfg.device)
    ...
    scores = rouge.compute(predictions=example_summaries, references=ground_truth_summaries)
